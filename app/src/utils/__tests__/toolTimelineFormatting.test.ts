...
describe('formatTimelineEntry', () => {
  ...
  it('formats file_read with shortened path', () => {
    expect(
      formatTimelineEntry(
        entry({
          name: 'file_read',
          argsBuffer: JSON.stringify({ path: 'crates/openhuman-core/src/openhuman/agent/progress.rs' }),
        })
      )
  ...
  it('formats list with directory path', () => {
    expect(
      formatTimelineEntry(
        entry({ name: 'list', argsBuffer: JSON.stringify({ path: 'crates/openhuman-core/src/openhuman/tools' }) })
      )
    ).toEqual({ title: 'Listing directory', detail: 'crates/openhuman-core/src/openhuman/tools' });
  });
...
describe('categorizeTool', () => {
  ...
  it('categorizes the canonical web-search name, not only its settings id', () => {
    ...
    // (`crates/openhuman-core/src/openhuman/tools/user_filter.rs:79-80`), and that is the name a
