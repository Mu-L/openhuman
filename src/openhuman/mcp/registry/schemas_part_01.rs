...
fn custom_server_schema(include_server_id: bool) -> ControllerSchema {
    let mut inputs = vec![
        FieldSchema {
            name: "display_name",
            ty: TypeSchema::String,
            comment: "User-visible name for the hand-entered server.",
            required: true,
        },
        FieldSchema {
            name: "transport",
            ty: TypeSchema::String,
            comment: "Transport: `stdio` or `http_remote`.",
            required: true,
        },
        FieldSchema {
            name: "command",
            ty: TypeSchema::Option(Box::new(TypeSchema::String)),
            comment: "Executable for a stdio server.",
            required: false,
        },
        FieldSchema {
            name: "args",
            ty: TypeSchema::Option(Box::new(TypeSchema::Array(Box::new(TypeSchema::String)))),
            comment: "Arguments for a stdio server.",
            required: false,
        },
        FieldSchema {
            name: "url",
            ty: TypeSchema::Option(Box::new(TypeSchema::String)),
            comment: "HTTP(S) endpoint for an http_remote server.",
            required: false,
        },
        FieldSchema {
            name: "env",
            ty: TypeSchema::Option(Box::new(TypeSchema::Map(Box::new(TypeSchema::String)))),
            comment: "Environment variables or HTTP headers; values are write-only.",
            required: false,
        },
        FieldSchema {
            name: "description",
    ...
    ControllerSchema {
        ...
        description: if include_server_id {
            "Update a hand-entered MCP server."
        } else {
            "Add a hand-entered MCP server."
        },
        inputs,
        outputs: vec![FieldSchema {
            name: "server",
            ty: TypeSchema::Ref("InstalledServer"),
            comment: "The saved server record; secret values are omitted.",
            required: true,
        }],
    }
}
