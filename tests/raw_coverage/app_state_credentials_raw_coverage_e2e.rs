...
use openhuman_core::security::credentials::ops::store_session;
...
async fn snapshot_clears_pending_backend_validation_after_successful_revalidation() {
    ...
    let harness = setup(&api_url);
    let config = harness.config().await;
    let active_user_root = openhuman_core::config::default_root_openhuman_dir()
        .expect("default openhuman root");
    ...
    assert_eq!(
        openhuman_core::config::read_active_user_id(&active_user_root).as_deref(),
        None,
...
async fn snapshot_activates_user_dir_after_pending_revalidation_without_initial_user_id() {
    ...
    let active_user_root = openhuman_core::config::default_root_openhuman_dir()
        .expect("default openhuman root");
    assert_eq!(
        openhuman_core::config::read_active_user_id(&active_user_root),
        None,
    ...
    let activated_user_dir = openhuman_core::config::user_openhuman_dir(
        &active_user_root,
    ...
    let active_config = openhuman_core::config::Config::load_from_default_paths()
        .await
...
async fn snapshot_preserves_default_active_user_when_env_scoped_revalidation_is_rejected() {
    ...
    let active_user_root = openhuman_core::config::default_root_openhuman_dir()
        .expect("default openhuman root");
    openhuman_core::config::write_active_user_id(&active_user_root, "desktop-user")
...
async fn snapshot_clears_supplied_user_pending_session_after_revalidation_rejection() {
    ...
    let active_user_root = openhuman_core::config::default_root_openhuman_dir()
        .expect("default openhuman root");
    openhuman_core::config::write_active_user_id(&active_user_root, user_id)
