...
`crates/openhuman-core/src/core/runtime/context.rs:608` calls `global::init(cfg.workspace_dir)`, and `:621` binds the module driver — same `if plan.memory` block, and both resolve to `<workspace>/memory/memory.db`. Two live `MemoryClient`s over one SQLite file.
...
cargo metadata --locked                           # and again for crates/openhuman-app — two Cargo worlds
```
