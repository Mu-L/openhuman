...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| Transport-agnostic dispatch | `ControllerSchema` + `invoke_method` (`crates/openhuman-core/src/core/jsonrpc.rs:230`) shared by HTTP `/rpc`, CLI `call`, generic namespace dispatch, and MCP stdio                                        | The _contract_ is host-neutral; only the bootstrap is not                                                                 |
...
| Headless server mode        | `openhuman-core run/serve` (`crates/openhuman-core/src/core/cli.rs:66`), `--jsonrpc-only`, Bearer token via `OPENHUMAN_CORE_TOKEN` (`crates/openhuman-core/src/core/auth.rs:160`)                                                | Cloud deployment of a _single_ core already works                                                                         |
| Host discrimination         | `HostKind { TauriShell, Cli, Docker }` (`crates/openhuman-core/src/core/types.rs:167`) threaded into `bootstrap_core_runtime`                                                                              | The natural seam for the refactor already exists                                                                          |
...
| --- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Bootstrap fused to transport       | `run_server_inner` (`crates/openhuman-core/src/core/jsonrpc.rs:1738`) + `bootstrap_core_runtime` (`:2428`)                                                                                                            | One function: controller registration, master key, token seeding, `Config::load_or_init`, ~10 global store inits (`:1807-1892`), port bind, env mutation, router, ready signal, inline `tokio::spawn` of every background service. You cannot get "just the dispatcher" |
| 2   | Handlers are bare `fn` pointers    | `ControllerHandler` (`crates/openhuman-core/src/core/all.rs:21`)                                                                                                                                                      | No captured state possible; every handler reads process globals                                                                                                                                                                                                         |
| 3   | Three hand-maintained registries   | `crates/openhuman-core/src/core/all.rs` — handlers (`:105-344`), schemas (`:375-509`), namespace descriptions (`:530-690`)                                                                                            | Parallel lists in `OnceLock` statics, panic-on-drift validation                                                                                                                                                                                                         |
| 4   | Process singletons                 | `RPC_TOKEN` (`crates/openhuman-core/src/core/auth.rs:75`), `GLOBAL_BUS` (`crates/openhuman-core/src/core/event_bus/bus.rs:20`), `NativeRegistry` (`crates/openhuman-core/src/core/event_bus/native_request.rs:329`), every `*::global::init(workspace_dir)` store | One of each per process                                                                                                                                                                                                                                                 |
...
| 9   | Dead dispatch tier                 | `crates/openhuman-core/src/rpc/dispatch.rs:10` always returns `None`                                                                                                                                                  | Removable noise                                                                                                                                                                                                                                                         |

### 1.3 Teams today

`src/openhuman/team/` is a **pure thin proxy** to `tinyhumansai/backend`
(session JWT via `crate::api::jwt`, URL via
`effective_backend_api_url`, `crates/openhuman-core/src/api/config.rs:154`). All membership, roles,
...
| CLI `call` / namespace dispatch (`crates/openhuman-core/src/core/cli.rs`) | `invoke_method(default_state(), …)` | `ServiceSet::none()` build → `runtime.invoke()` — no port bound for one-shot calls                                              |
| MCP stdio (`crates/openhuman-core/src/core/cli.rs` `mcp`)                 | same funnel                         | transport adapter over `runtime.invoke()`                                                                                       |
