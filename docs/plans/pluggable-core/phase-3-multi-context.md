...
| Item                         | Today                                                                                                                                 | After                                                                                                                                                                  |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RPC bearer                   | `RPC_TOKEN: OnceLock<String>` (`crates/openhuman-core/src/core/auth.rs:75`)                                                                                 | field on `CoreContext` — it gates a per-runtime HTTP listener; `auth::get_rpc_token` facade reads the default context                                                  |
...
| Event-bus domain subscribers | `register_domain_subscribers` under `std::sync::Once` (`crates/openhuman-core/src/core/jsonrpc.rs:2232`)                                                    | registration keyed per context (subscription handles owned by `CoreContext`, dropped on shutdown); process-level dedupe kept only for genuinely process-global targets |
