...
4. ~~Reconcile `subagent_sessions/` (durable reusable sessions) with crate `JsonlTaskStore`/checkpoint reuse instead of a parallel JSON store.~~ **Premise wrong (2026-08-30 audit):** `subagent_sessions/` is not a parallel task store. It is a reuse-selector directory keyed on ~8 product dimensions (`SubagentSessionSelector`, `subagent_sessions/types.rs:37-45`: `parent_session`, `parent_thread_id`, `agent_id`, `toolkit`, `model`, `sandbox_mode`, `action_root`, `task_key`) holding host `ChatMessage` transcripts, with a reusability status (`DurableSubagentStatus` — `Running`/`Idle`/`AwaitingUser`/`Failed`/`Closed`, `types.rs:8-32`) that has no crate counterpart. The crate's `JsonlTaskStore` (`vendor/tinyagents/src/graph/orchestration/store.rs`) is a durable task-status append log, an orthogonal concept. Reclassified **HOST-OWNED** — no reconciliation needed.
5. ~~Thin `spawn_parallel_graph.rs` further into the seam ...; fix the two-spawn-path inconsistency (worktree diff results threaded in one path, hardcoded empty in the other — `ops.rs:559-562`).~~ **Stale/resolved (2026-08-30 audit):** the line reference is stale and the inconsistency is gone. Only `spawn_parallel_graph.rs::create_spawn_parallel_worktree` (currently ~lines 318-407) calls `worktree::OpenHumanWorktreeIsolation::new(...).prepare(...)`; every other spawn path inherits a descriptor and correctly reports `worktree_path: None` (`tools/dispatch.rs`, `tools/spawn_async_subagent.rs`, `tools/continue_subagent.rs`), which is the correct behavior, not a bug. Item resolved; no further action.
