...
// The failure-cache test fixtures live in the backoff suite, which owns that
// global. Borrowing its lock rather than declaring a second one is the point:
// two locks over one static is exactly the interference this suite hit.
use super::super::current_user_backoff_tests::{
    seed_current_user_failure, CurrentUserFailureResetGuard, CurrentUserStalenessResetGuard,
    CURRENT_USER_FAILURE_TEST_LOCK,
};
...
fn forget_current_user_caches_clears_the_freshness_stamp() {
    ...
    assert!(
        current_user_staleness(api_base, token).1.is_some(),
        "precondition: the stamp must be set for this test to mean anything"
    );

    forget_current_user_caches();

    assert_eq!(
        current_user_staleness(api_base, token).1,
        None,
        "the pre-logout freshness stamp survived sign-out, so the next account \
         would be told this one's data age"
    );
}
...
async fn a_successful_refresh_that_races_sign_out_does_not_repopulate_the_snapshot() {
    ...
    let generation = current_user_generation();
    // Resolved before `config` moves into the task; the freshness stamp is keyed
    // on it, so the assertion at the end needs the same string the fetch used.
    let api_base_probe = current_user_api_base(&config);
    ...
    assert_eq!(
        current_user_staleness(&api_base_probe, "jwt-race").1,
        None,
        "the raced refresh stamped the signed-out identity as freshly loaded"
    );
}
