...
async fn propose_workflow_rejects_an_incompatible_saved_child_reference() {
    ...
    let child_graph = crate::flows::ops::migrate_and_deserialize_graph(legacy_child)
    ...
    let child = crate::flows::store::create_flow(
        &config,
...
fn propose_workflow_description_matches_typed_node_contracts() {
    ...
    for contract in crate::flows::all_node_kind_contracts() {
        assert!(
