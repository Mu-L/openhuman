...
fn handle_inference_resolve_model(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let resolved = crate::inference::provider::factory::resolve_model_for_hint(
            &p.hint, &config,
        );
        ...
        let vision =
            crate::inference::model_context::model_supports_vision(&resolved, &config);
...
fn handle_inference_get_client_config(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        to_json(crate::inference::rpc::inference_get_client_config().await?)
    })
}
