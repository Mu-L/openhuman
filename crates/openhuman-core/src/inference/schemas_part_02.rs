...
fn handle_inference_list_models(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::inference::rpc::inference_list_models(&request.provider_id).await?,
        )
    })
}
...
fn handle_inference_device_profile(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(
        async move { to_json(crate::inference::rpc::inference_device_profile().await?) },
    )
}

fn handle_inference_provider_auth_errors(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        to_json(crate::inference::rpc::inference_provider_auth_errors().await?)
    })
}
...
fn handle_inference_openai_oauth_import_codex_cli(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::rpc::inference_openai_oauth_import_codex_cli(&config)
                .await?,
        )
    })
}
...
fn handle_inference_summarize(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::rpc::inference_summarize(&config, &p.text, p.max_tokens)
                .await?,
        )
    })
}
...
fn handle_inference_prompt(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::rpc::inference_prompt(
                &config,
                &p.prompt,
                p.max_tokens,
                p.no_think,
...
fn handle_inference_should_react(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::rpc::inference_should_react(
                &config,
                &p.message,
                &p.channel_type,
            )
...
fn handle_inference_analyze_sentiment(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::inference::rpc::inference_analyze_sentiment(&config, &p.message)
                .await?,
        )
    })
}
