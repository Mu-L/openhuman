...
fn python_needed_eagerly(config: &Config) -> bool {
    config.runtime_python.enabled
        && !crate::runtime::python_server::enabled_backends(config).is_empty()
}
...
async fn runtime_python_server_is_done(config: &Config) -> bool {
    if crate::runtime::python_server::enabled_backends(config).is_empty() {
        return true;
    }
    let status = crate::runtime::python_server::status().await;
    status.running
}
...
async fn runtime_python_server_run(config: &Config) -> Result<(), String> {
    ...
    crate::runtime::python_server::ensure_started(config)
        .await
        .map(|_| {
...
async fn kompress_is_done(config: &Config) -> bool {
    ...
    crate::runtime::python_server::kompress_provisioned(config)
}
...
#[cfg(feature = "runtime-node")]
fn build_node_bootstrap(config: &Config) -> crate::runtime::node::NodeBootstrap {
    crate::runtime::node::NodeBootstrap::new(std::sync::Arc::new(config.clone()))
}
