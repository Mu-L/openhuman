...
pub(crate) async fn run_turn_via_tinyagents_shared(
    ...
    if let Some(session_kv) =
        crate::agent::session_import::live::session_kv_store().await
    {
    ...
    let steer_thread_label = subagent_scope
        ...
        .or_else(|| match crate::agent::turn_origin::current() {
            Some(crate::agent::turn_origin::AgentTurnOrigin::WebChat {
                thread_id,
                ..
            }) => Some(thread_id),
        ...
        Err(e) => {
            ...
            if let tinyagents_harness::TinyAgentsError::LimitExceeded(msg) = &e {
                if msg.contains("model call") {
                    ...
                    return Err(anyhow::Error::new(
                        crate::agent::error::AgentError::MaxIterationsExceeded {
                            max: max_iterations,
                        },
                    ));
                }
    ...
    let conversation = crate::agent::message_convert::messages_to_conversation(
        crate::agent::message_convert::messages_since_request(
            &run.messages,
            request_base_len,
        ),
    );
