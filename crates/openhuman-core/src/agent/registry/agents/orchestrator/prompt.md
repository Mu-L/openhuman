...
Before searching elsewhere, check **Connected MCP Servers**. If one can answer, use `mcp_registry_list_tools` for its schema, then call `mcp_registry_tool_call` yourself. Use `mcp_registry_status` when connection state is unclear and `mcp_registry_connect` only for an installed, enabled server that needs reconnecting. Never guess a server tool's arguments.<!--route:mcp-->
