...
pub(crate) fn classify_inference_error(err: &str) -> ClassifiedError {
    ...
    let classified = if err.to_ascii_lowercase().contains(CODEX_SENTINEL) {
        ...
        // to quote.
        //
        // An empty successful completion is not a billing verdict. In
        // particular, reasoning-only replies can spend output tokens but
        // produce no visible answer. Reserve credit guidance for explicit
        // backend/provider errors; this path has neither an error code nor
        // a response body establishing the user's balance.
        ClassifiedError {
            error_type: "empty_response",
            message: "The model returned an empty response. Please retry. If it keeps happening, \
