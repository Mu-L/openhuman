...
- `crates/openhuman-core/src/openhuman/agent/` — `agent` job type runs through `agent::triage::TriggerEnvelope::from_cron` + `apply_decision`.
- `crates/openhuman-core/src/openhuman/security/` — `SecurityPolicy::from_config` sandboxes shell jobs.
- `crates/openhuman-core/src/openhuman/config/` — `Config` provides poll interval, workspace dir, autonomy policy.
- `crates/openhuman-core/src/openhuman/platform/health/` — `health::bus::register_health_subscriber` on startup.
- `crates/openhuman-core/src/openhuman/channels/` — `bus.rs` can fan delivery events into channels.
- `crates/openhuman-core/src/core/event_bus/` — `init_global`, `publish_global(DomainEvent::Cron(*))`.

## Called by

- `crates/openhuman-core/src/openhuman/tools/impl/system/schedule.rs` — `schedule` tool exposes cron operations to agents.
