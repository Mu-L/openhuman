...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/util/tls/mod.rs` | Entire module. Module docstring (policy) + the single `tls_client_builder()` function. No `mod`/`pub mod` decls; no submodules. |
...
- `crates/openhuman-core/src/openhuman/config/schema/proxy.rs` — proxy-aware client builders (primary + fallback).
- `crates/openhuman-core/src/openhuman/integrations/client.rs` and integration tools (`tools/searxng.rs`, `tools/brave.rs`, `tools/querit.rs`, `tools/seltz.rs`).
- `crates/openhuman-core/src/openhuman/inference/provider/compatible.rs` — OpenAI-compatible inference provider clients.
- `crates/openhuman-core/src/openhuman/integrations/composio/client.rs`, `crates/openhuman-core/src/openhuman/desktop/app_state/ops.rs`.
- `crates/openhuman-core/src/api/rest.rs` (REST API client).

Registered in the domain tree via `pub mod tls;` in `crates/openhuman-core/src/openhuman/mod.rs`.
