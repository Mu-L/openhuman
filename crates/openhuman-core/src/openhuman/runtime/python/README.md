...
- `crates/openhuman-core/src/openhuman/runtime/python_server/` — resolves an interpreter, then spawns
  and supervises the long-lived model server with `spawn_stdio`.
- `crates/openhuman-core/src/openhuman/tools/impl/system/{python_exec,shell}.rs` — hold an
  `Arc<PythonBootstrap>`; `python_exec` calls `resolve()`, `shell` uses the
  non-blocking `try_cached()` for `PATH` injection.
- `crates/openhuman-core/src/openhuman/skills/runtime/ops.rs` — resolves an interpreter for
  Python-backed skills.
- `crates/openhuman-core/src/openhuman/agent/harness_init/registry.rs` — the Python init step uses
