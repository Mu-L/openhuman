...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/platform/health/mod.rs` | Export-only: declares modules, re-exports `core::*`, `ops::*`, `pub use ops as rpc`, and the controller-schema pair (`all_health_controller_schemas` / `all_health_registered_controllers`). |
| `crates/openhuman-core/src/openhuman/platform/health/core.rs` | The registry itself: `ComponentHealth`/`HealthSnapshot` types, the `OnceLock<HealthRegistry>` singleton (`started_at` + `Mutex<BTreeMap>`), and the mutators/`snapshot`/`snapshot_json` functions. |
| `crates/openhuman-core/src/openhuman/platform/health/ops.rs` | RPC handler logic returning `RpcOutcome<T>`: `health_snapshot()` and `system_info()` (+ `SystemInfo` type). |
| `crates/openhuman-core/src/openhuman/platform/health/schemas.rs` | Controller schemas + `handle_snapshot`/`handle_system_info` async handlers that delegate to `ops` and serialize via `into_cli_compatible_json`. |
| `crates/openhuman-core/src/openhuman/platform/health/bus.rs` | `HealthSubscriber` (`EventHandler`) and `register_health_subscriber()`; maps domain events to registry mutations. |
...
- `crates/openhuman-core/src/openhuman/channels/runtime/{startup,supervision}.rs` and `crates/openhuman-core/src/openhuman/channels/tests/health.rs` — channel runtime updates component health.
- `crates/openhuman-core/src/openhuman/cron/scheduler.rs`, `crates/openhuman-core/src/openhuman/platform/update/scheduler.rs` — emit/consume health signals.
