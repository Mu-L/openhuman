...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/platform/startup/mod.rs` | Export-only: module docstring + `pub mod ops;` + re-export of `run_workspace_migrations`. |
| `crates/openhuman-core/src/openhuman/platform/startup/ops.rs` | Implementation of `run_workspace_migrations` — orchestrates the two workspace migrations and logging. |
