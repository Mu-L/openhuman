...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/voice/audio_toolkit/mod.rs` | Export-only module surface: re-exports ops fns, schema registries, and types. |
| `crates/openhuman-core/src/openhuman/voice/audio_toolkit/types.rs` | Serde domain types: `AudioFormat`, request/result structs. |
| `crates/openhuman-core/src/openhuman/voice/audio_toolkit/ops.rs` | Business logic: `generate_podcast`, `email_podcast`, `generate_and_email_podcast`, `resolve_email_capture_dir`, plus private helpers (path/format/voice resolution, base64 decode, MIME enforcement, slugify, email build/capture). Contains the unit-test suite. |
| `crates/openhuman-core/src/openhuman/voice/audio_toolkit/schemas.rs` | RPC controller schemas + `handle_*` handlers delegating to `ops.rs`; loads `Config` via `config::rpc`. |
| `crates/openhuman-core/src/openhuman/voice/audio_toolkit/tools.rs` | Re-exports the three agent tool structs from `tools/podcast.rs`. |
| `crates/openhuman-core/src/openhuman/voice/audio_toolkit/tools/podcast.rs` | `Tool` impls: `AudioGeneratePodcastTool`, `AudioEmailPodcastTool`, `AudioGenerateAndEmailPodcastTool`; security gating + arg parsing. |
...
Tools are constructed with `Arc<Config>` + `Arc<SecurityPolicy>` and registered in `crates/openhuman-core/src/openhuman/tools/ops.rs`; re-exported via `crates/openhuman-core/src/openhuman/tools/mod.rs`.
...
- `crates/openhuman-core/src/openhuman/tools/ops.rs` — instantiates the three tools into the agent tool registry; `crates/openhuman-core/src/openhuman/tools/mod.rs` re-exports them.
