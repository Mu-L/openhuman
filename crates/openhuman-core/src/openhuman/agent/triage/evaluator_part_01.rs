...
///
/// Canonical sources:
/// - `crates/openhuman-core/src/openhuman/agent/bus.rs` — `Blocked` / `ReviewBlocked` arms of the
///   `enforce_prompt_input` decision (the path the triage evaluator hits via
///   `agent.run_turn`).
/// - `crates/openhuman-core/src/openhuman/agent/harness/session/runtime.rs` — same strings in the
///   tool-call loop, kept identical so this classifier covers both.
/// - `crates/openhuman-core/src/openhuman/inference/local/ops.rs` — user-facing variants with the
