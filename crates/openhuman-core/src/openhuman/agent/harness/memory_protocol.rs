...
///   (`crates/openhuman-core/src/openhuman/tools/impl/filesystem/update_memory_md.rs`); only a
///   `MEMORY.md` edit reconciles the memory index, so a `SKILL.md` edit is not
///   an [`MemoryOp::IndexUpdate`] and must not close the cycle.
/// - the consolidated `memory_tree` tool (`crates/openhuman-core/src/openhuman/memory/query/mod.rs`)
