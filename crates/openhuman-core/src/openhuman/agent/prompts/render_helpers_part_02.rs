...
pub fn default_workspace_file_content(filename: &str) -> &'static str {
    // The bundled identity files live at `crates/openhuman-core/src/openhuman/agent/prompts/`
    // (owned by the `agent/` tree because they describe agent identity).
    // This module is under `crates/openhuman-core/src/openhuman/agent/context/`, so the relative path
