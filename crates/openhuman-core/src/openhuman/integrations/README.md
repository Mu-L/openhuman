...
Search provider implementations live under `crates/openhuman-core/src/openhuman/search/`. This module
...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/integrations/mod.rs` | Export-only module root. Re-exports `build_client`, `pricing_for_config`, `IntegrationClient`, pricing/envelope types, and `ToolScope`. |
| `crates/openhuman-core/src/openhuman/integrations/client.rs` | `IntegrationClient` + `post`/`get`/`pricing`, backend URL sanitization, and client construction. |
| `crates/openhuman-core/src/openhuman/integrations/types.rs` | Shared serde types for backend envelopes and pricing. |
| `crates/openhuman-core/src/openhuman/integrations/tools.rs` | Aggregates and re-exports non-search integration tool modules. |
| `crates/openhuman-core/src/openhuman/integrations/tools/apify.rs` | Apify actor run + status + dataset results. |
| `crates/openhuman-core/src/openhuman/integrations/tools/google_places.rs` | Google Places search + details. |
| `crates/openhuman-core/src/openhuman/integrations/tools/stock_prices.rs` | Market data via backend financial APIs. |
| `crates/openhuman-core/src/openhuman/integrations/tools/twilio.rs` | Outbound phone calls via backend Twilio. |

## Search Boundary

Search-owned tools are in `crates/openhuman-core/src/openhuman/search/tools/`, including Parallel,
Brave, Querit, SearXNG, Seltz, TinyFish, and the managed `WebSearchTool`.
The search registry in `crates/openhuman-core/src/openhuman/search/registry.rs` decides which search
tool surface is active for `search.engine`.

## Public Surface

From `crates/openhuman-core/src/openhuman/integrations/mod.rs`:
...
Tools are constructed and registered by `crates/openhuman-core/src/openhuman/tools/ops.rs`, gated by
...
by `crates/openhuman-core/src/openhuman/search/`.
...
- Direct search APIs such as SearXNG, Brave, Querit, and Seltz are intentionally
  outside this module in `crates/openhuman-core/src/openhuman/search/`.
