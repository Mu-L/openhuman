...
- `crates/openhuman-core/src/openhuman/config/` — `Config` for workspace-relative data directory.
- `crates/openhuman-core/src/openhuman/security/credentials/` — `credentials::rpc::{encrypt_secret, decrypt_secret}` carry the actual key-management responsibility.

## Called by

- `crates/openhuman-core/src/openhuman/security/credentials/` — uses the same `EncryptedPayload` / `EncryptionKey` primitives directly when storing per-channel secrets.
- `crates/openhuman-core/src/core/all.rs` — registers `all_encryption_*` controllers so the shell + CLI can encrypt configuration secrets.
- Indirect: `crates/openhuman-core/src/openhuman/memory/`, `crates/openhuman-core/src/openhuman/channels/`, and `crates/openhuman-core/src/openhuman/inference/local/` rely on the credentials domain (which in turn uses this layer) for secrets at rest.

## Tests

- This domain has no `*_tests.rs` siblings; the underlying crypto round-trips are exercised by `crates/openhuman-core/src/openhuman/security/keyring/encrypted_store_tests.rs` and the credentials tests, which both cover encrypt/decrypt happy paths and tampered-ciphertext rejection.
