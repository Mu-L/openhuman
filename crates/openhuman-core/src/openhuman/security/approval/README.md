...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/security/approval/mod.rs` | Export-focused: module docstring, `pub mod` decls, `pub use` re-exports including the controller-schema pair. |
| `crates/openhuman-core/src/openhuman/security/approval/gate.rs` | `ApprovalGate` — the singleton coordinator. `init_global`/`try_global`, `intercept`/`intercept_audited`, `decide`, `record_execution`, `list_pending`, `list_recent_decisions`, the thread→request routing map, `ApprovalChatContext` task-local, and `parse_approval_reply`. |
| `crates/openhuman-core/src/openhuman/security/approval/store.rs` | SQLite persistence (`pending_approvals` table). `insert_pending`, `decide`, `get_decision`, `record_execution`, `list_pending`, `list_recent_decisions`, `purge_session`, `expire_stale`, plus idempotent column migration for the v1 schema. |
| `crates/openhuman-core/src/openhuman/security/approval/types.rs` | Serde domain types: `PendingApproval`, `ApprovalAuditEntry`, `ApprovalDecision`, `GateOutcome`, `ExecutionOutcome`. |
| `crates/openhuman-core/src/openhuman/security/approval/redact.rs` | `redact_args` (PII/chat-content key scrubbing + home-path stripping) and `summarize_action` (safe-field summary). |
| `crates/openhuman-core/src/openhuman/security/approval/rpc.rs` | Domain RPC entry points returning `RpcOutcome<T>`: `approval_list_pending`, `approval_list_recent_decisions`, `approval_decide`. |
| `crates/openhuman-core/src/openhuman/security/approval/schemas.rs` | Controller schemas + `handle_*` fns wiring the RPC into the registry. |
...
- `DomainEvent::ApprovalRequested { request_id, tool_name, action_summary, args_redacted, session_id, thread_id, client_id }` — emitted when a call is parked. Bridged to the `approval_request` web-channel socket event by `ApprovalSurfaceSubscriber` (defined in `crates/openhuman-core/src/openhuman/web_chat/`).
...
- `crates/openhuman-core/src/openhuman/agent/tinyagents/middleware.rs` (`ApprovalSecurityMiddleware`, a `wrap_tool` middleware on every turn path) — routes external-effect tool calls through the gate before `execute()` and records the terminal audit row.
- `crates/openhuman-core/src/openhuman/web_chat/` — sets `APPROVAL_CHAT_CONTEXT`, hosts `ApprovalSurfaceSubscriber`, and routes typed yes/no replies to `approval_decide`.
- `crates/openhuman-core/src/openhuman/channels/proactive.rs`, `crates/openhuman-core/src/openhuman/agent/triage/escalation.rs`, `crates/openhuman-core/src/openhuman/tools/impl/system/install_tool.rs`, `crates/openhuman-core/src/openhuman/web3/wallet/execution.rs` — interact with the gate / approval types.
