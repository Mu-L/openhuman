...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/openhuman/mcp/audit/mod.rs` | Export-only module surface; re-exports store fns, types, and the controller-schema pair. |
| `crates/openhuman-core/src/openhuman/mcp/audit/types.rs` | Serde types: `NewMcpWriteRecord` (insert), `McpWriteRecord` (row), `McpWriteListQuery` (filters/paging). |
| `crates/openhuman-core/src/openhuman/mcp/audit/store.rs` | Persistence: `record_write` (INSERT) and `list_writes` (filtered SELECT), arg/error serialization, limit/offset normalization, row mapping. Includes the test suite. |
| `crates/openhuman-core/src/openhuman/mcp/audit/schemas.rs` | Controller schema for `mcp_audit.list` + `handle_list` handler delegating to `store::list_writes`. Internal-controller registration. |
...
- Table `mcp_writes` in the memory-tree chunk SQLite DB; schema and indexes (`idx_mcp_writes_timestamp`, `idx_mcp_writes_client`, `idx_mcp_writes_tool`) are created in `crates/openhuman-core/src/openhuman/memory/store/chunks/store.rs`, not here.
...
- `crates/openhuman-core/src/openhuman/mcp/server/write_dispatch.rs` — calls `mcp::audit::record_write` after each write attempt and `mcp::audit::list_writes` for its own surfaces/tests.
- `crates/openhuman-core/src/openhuman/mcp/server/tools_tests.rs` — reads back audit rows in tests.
- `crates/openhuman-core/src/core/all.rs` — registers the internal controller.
- `crates/openhuman-core/src/openhuman/tools/registry/ops.rs` — independently `COUNT`s `mcp_writes` rows (queries the shared table directly, not via this module's API).
