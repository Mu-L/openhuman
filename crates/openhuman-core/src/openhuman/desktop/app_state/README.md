...
- `crates/openhuman-core/src/openhuman/config/` — `config_rpc::*` for `Config` reads and the workspace dir resolver.
- `crates/openhuman-core/src/openhuman/autocomplete/` — `AutocompleteStatus` snapshot.
- `crates/openhuman-core/src/openhuman/inference/local/` — `LocalAiStatus` snapshot.
- `crates/openhuman-core/src/openhuman/platform/service/` — `ServiceState` / `ServiceStatus` runtime info.
- `crates/openhuman-core/src/openhuman/security/credentials/` — `session_support::build_session_state` for the auth slice.
- `crates/openhuman-core/src/api/{config,jwt}` — backend base URL + bearer token used by the cached current-user fetch.

## Called by

- `crates/openhuman-core/src/openhuman/agent/harness/session/builder.rs` — agent builder reads cached app state when resolving identity.
