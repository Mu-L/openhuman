...
use crate::memory::api::provider::retrieval::{RetrievalNodeKind, RetrievalResponse};
...
async fn from_guard_over_a_driver_without_retrieval_stays_silent() {
    ...
    let inner: Arc<dyn crate::memory::api::provider::MemoryProvider> =
        Arc::new(tinymemory_api::null::NullMemoryProvider);
    let guard = crate::memory::guard::MemoryGuard::new(inner, Arc::new(embedded_policy()));
    let lane = AutoRecall::from_guard(Arc::new(guard));
    assert!(lane.enabled());
    assert!(lane.block_for(QUESTION).await.is_none());
}
