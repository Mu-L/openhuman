...
//!   `pub(in crate::memory)` with a single in-family caller, so
...
//!   `pub(in crate::memory)`, so even a re-export under another name
...
const ALLOWED: &[(&str, &str, &str)] = &[
    ...
    (
        "crates/openhuman-core/src/agent/learning/startup.rs",
        "binding::for_workspace(",
    ...
    (
        "crates/openhuman-core/src/memory/binding.rs",
        "NullMemoryProvider::new(",
    ...
    (
        "crates/openhuman-core/src/memory/ops/provider.rs",
        ".memory_binding(",
    ...
    (
        ...
        ".profile_conn(",
        "sole in-family call; wraps the raw handle in ProfileStore. profile_conn is pub(in crate::memory), so the compiler — not this lint — is the primary enforcement",
    ),
...
fn scan() -> BTreeSet<(String, String)> {
    ...
    // Scan the complete core package: the domain and runtime composition
    // layers now share one crate-native source tree.
    collect_rs_files(
