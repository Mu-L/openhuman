...
use crate::memory::api::provider::chunks::SourceIngestQuery;
...
impl SourceStatus {
    ...
    pub(crate) fn with_live_sync(
        mut self,
        live: Option<crate::memory::sync_activity::LiveSync>,
    ) -> Self {
        match live {
            Some(live) => {
...
pub async fn status_list(config: &Config) -> Result<Vec<SourceStatus>, String> {
    ...
    if sources.is_empty() {
        ...
        crate::memory::sync_activity::prune_stale();
        return Ok(Vec::new());
    }
    ...
    Ok(sources
        ...
        .map(|source| {
            ...
            let live = crate::memory::sync_activity::live_sync(&source.id);
...
) -> Vec<crate::memory::api::provider::chunks::SourceIngestStatus> {
