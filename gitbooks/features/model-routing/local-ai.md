...
| ------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| **Memory embeddings**     | `bge-m3`                          | `crates/openhuman-core/src/openhuman/inference/embeddings/ollama.rs` - used by the [Memory Tree](../obsidian-wiki/memory-tree.md) for vector search. |
| **Summary-tree building** | `gemma3:1b-it-qat` (configurable) | `crates/openhuman-core/src/openhuman/tree_summarizer/ops.rs` - source / topic / global summary builders for the Memory Tree.                         |
| **Heartbeat loop**        | small chat model                  | `crates/openhuman-core/src/openhuman/subconscious/heartbeat/` - periodic background reflection.                                                      |
| **Learning / reflection** | small chat model                  | `crates/openhuman-core/src/openhuman/agent/learning/reflection.rs` - passes that consolidate what was learned.                                       |
| **Subconscious**          | small chat model                  | `crates/openhuman-core/src/openhuman/subconscious/executor.rs` - background evaluation loop.                                                         |
| **Chat**                  | configured local chat model       | `Config::workload_local_model("chat")` reads `chat_provider`; `crates/openhuman-core/src/openhuman/routing/provider.rs` handles hint routing.        |
...
| **STT**        | Backend-proxied transcription (`crates/openhuman-core/src/openhuman/voice/cloud_transcribe.rs`).                     |
...
- The `OpenAiCompatibleProvider` (`crates/openhuman-core/src/openhuman/providers/compatible.rs`) wraps Ollama exactly the way it wraps a remote OpenAI-style provider. No special-case code path.
...
Local runtime startup is gated in the core config (`crates/openhuman-core/src/openhuman/config/schema/local_ai.rs`):
...
OpenHuman handles the rest: lifecycle (`crates/openhuman-core/src/openhuman/inference/local/service/`), API clients, health checks, and graceful fallback to remote when the local provider disappears.
