...
| **Repo root `src/`**        | Rust **`openhuman_core`** library + **`openhuman-core`** CLI binary - core server, JSON-RPC, first-class JavaScript runtime (`crates/openhuman-core/src/openhuman/runtime/javascript/`) backed by a managed Node.js implementation, channels, memory, etc.                                                                                                                                                       |
| **`Cargo.toml`** (root)     | Builds the `openhuman-core` binary (`cargo build --bin openhuman-core`) staged into `crates/openhuman-app/binaries/` for the desktop bundle                                                                                                                                                                                                                                                       |
| **`crates/openhuman-core/src/openhuman/skills/`** | **Metadata-only** skill helpers (`ops_create`, `ops_discover`, `ops_install`, `ops_parse`, `inject`, `schemas`, `types`). The legacy QuickJS / `rquickjs` skill execution runtime was removed; skills now contribute metadata + tool descriptors that get injected into agent prompts, while tool execution flows through native Rust handlers and Node-backed helpers via `runtime_node`. |
...
| Domain                          | Role                                                                                                                                                                  |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `crates/openhuman-core/src/openhuman/skills/`         | Skill metadata: create/discover/install/parse `SKILL.md`, inject descriptors into agent prompts (`ops_create`, `ops_discover`, `ops_install`, `ops_parse`, `inject`). |
| `crates/openhuman-core/src/openhuman/skills/catalog/` | Registry of installed skills.                                                                                                                                         |
| `crates/openhuman-core/src/openhuman/skills/runtime/` | Execution of installed `SKILL.md` workflows: starts/cancels runs, reads run metadata/logs, resolves language runtimes, hosts the built-in `skill_executor` agent.     |
...
- **Tool sandboxing**: Executable tools run through `SecurityPolicy` (`crates/openhuman-core/src/openhuman/security/policy.rs`) and a host-appropriate sandbox backend selected at runtime: Docker, Bubblewrap, Firejail, Landlock, or Noop (`crates/openhuman-core/src/openhuman/security/{docker,bubblewrap,firejail,landlock}.rs`, `detect.rs`). The legacy per-skill QuickJS memory/stack limit model is gone
...
- **Prompt injection guard**: User prompts are normalized/scored and enforced server-side (`allow | review | block`) before model/tool execution. See `crates/openhuman-core/src/openhuman/security/prompt_injection/`
...
- **Agent engine on tinyagents** — every agent turn runs through the `tinyagents` crate harness via the seam in `crates/openhuman-core/src/openhuman/agent/tinyagents/`; see [Agent Harness](architecture/agent-harness.md).
- **Memory on tinycortex** — the generic store/tree/queue/retrieval/sync engine is crate-owned. OpenHuman keeps RPC, tools, scheduling, credentials, security/event policy, worker orchestration, and the host namespace-document store; `crates/openhuman-core/src/openhuman/memory/tinycortex/` implements those seams. Concrete embedding transports are shared through `tinyagents::harness::embeddings`.
...
| Path                              | Purpose                                                 |
| --------------------------------- | ------------------------------------------------------- |
| `crates/openhuman-core/src/openhuman/security/devices/` | Rust devices domain (pairing, store, crypto, event bus) |
