...
> Rust traits in `crates/openhuman-core/src/openhuman/agent/hooks.rs` that an *embedding host* installs
...
`crates/openhuman-core/src/openhuman/hooks/` — `types` (the wire contract), `config` (the file and its
