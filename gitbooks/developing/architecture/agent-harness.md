...
> harness** via the adapter seam in [`crates/openhuman-core/src/openhuman/agent/tinyagents/`](../../../crates/openhuman-core/src/openhuman/agent/tinyagents/)
...
> through a `StopHookMiddleware` ([`tinyagents/stop_hooks.rs`](../../../crates/openhuman-core/src/openhuman/agent/tinyagents/stop_hooks.rs))
...
> [`tinyagents/orchestration.rs`](../../../crates/openhuman-core/src/openhuman/agent/tinyagents/orchestration.rs):
>
> - [`tinyagents/delegation.rs`](../../../crates/openhuman-core/src/openhuman/agent/tinyagents/delegation.rs)
...
>   (`execute → complete | fail → done`, [`agent_teams/graph.rs`](../../../crates/openhuman-core/src/openhuman/agent/orchestration/agent_teams/graph.rs));
...
The older `ToolDispatcher` trait (`crates/openhuman-core/src/openhuman/agent/dispatcher.rs`) with its three dialects still exists, but as a **transcript-compatibility layer**, not a live routing choice:
...
Two directories under the agent's existing `action_dir` at runtime (the implementation lives in `crates/openhuman-core/src/openhuman/agent/harness/artifact_offload/`):
...
1. **Loader-time (static).** [`agents::loader::validate_tier_hierarchy`](../../../crates/openhuman-core/src/openhuman/agent/agents/loader.rs) runs over the merged registry (built-ins + workspace TOMLs) and refuses to boot a registry that lists a same-tier or worker-with-subagents entry. Built-in archetypes are checked at compile-test time; user-shipped TOMLs are checked at workspace load.
...
The domain lives in [`crates/openhuman-core/src/openhuman/flows/rhai/`](../../../crates/openhuman-core/src/openhuman/flows/rhai/README.md):
...
The harness shell lives under `crates/openhuman-core/src/openhuman/agent/`, with the tinyagents adapter seam in `crates/openhuman-core/src/openhuman/agent/tinyagents/` and archetype definitions in `crates/openhuman-core/src/openhuman/agent/registry/`. The README in `crates/openhuman-core/src/openhuman/agent/` enumerates the public surface; the most load-bearing files (paths relative to `crates/openhuman-core/src/openhuman/agent/` unless prefixed) are:
...
> The bespoke `crates/openhuman-core/src/openhuman/agent_graph/` engine, `GraphBlueprint`, and the
...
Alongside the linear tool-call loop, the harness ships a **LangGraph-style state-machine engine** under [`crates/openhuman-core/src/openhuman/agent_graph/`](../../../crates/openhuman-core/src/openhuman/agent_graph/) (issue #4249). Where the loop is an implicit "prompt → tool → result → next prompt" cycle, a graph models agent execution as an explicit directed graph of **nodes** (states) and **edges** (transitions), with typed working state that survives across transitions, parallel branches, and checkpoints.
...
Each agent folder under `crates/openhuman-core/src/openhuman/agent/registry/agents/<name>/` (and the four agents that live in their own domains) now contains, alongside `agent.toml` + `prompt.rs`:
...
> **Status (issue #4249, superseded by the published `tinyagents` crate):** the in-house `agent_graph` engine described in this section **no longer exists**. openhuman's agent engine + orchestration now run on the published [`tinyagents`](https://crates.io/crates/tinyagents) **2.1** crate (the same LangGraph-style harness + durable graph runtime), via the adapter seam in `crates/openhuman-core/src/openhuman/agent/tinyagents/`. The sections above are retained as design history; the subsection below describes the live architecture.
...
| File (`crates/openhuman-core/src/openhuman/agent/tinyagents/`)          | Role                                                                                                                                                                                                                                                                           |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
**Orchestration on graphs** (`crates/openhuman-core/src/openhuman/agent/orchestration/`):
